...
hulken.getAvgResponseTime = function() {
  ...
  return (totalRespTime / hulken.executedRequests.length);
};
...
hulken._setDefaults = function() {
  ...
  hulken.settings.requestsArray = undefined;
  ...
  hulken.settings.returnAllRequests = false;
  hulken.settings.headers = {};
  hulken.settings.requestValueLists = {};
  hulken.settings.printLoadDistribution = false;
};
hulken._overrideDefaultsWithOptions = function(options) {
  if (options) {
    ...
    if (options.returnAllRequests) {
      hulken.settings.returnAllRequests = options.returnAllRequests;
      printer.print('returnAllRequests set to '.cyan +
        hulken.settings.returnAllRequests.toString().bold.magenta);
    }
    if(options.headers){
      hulken.settings.headers = options.headers;
      printer.print('headers set to '.cyan +
        JSON.stringify(hulken.settings.headers).bold.magenta);
    }
    if(options.requestValueLists){
      hulken.settings.requestValueLists = options.requestValueLists;
      printer.print('requests value lists object set to '.cyan +
        JSON.stringify(hulken.settings.requestValueLists).bold.magenta);
    }
    if(options.requestsArray){
      hulken.settings.requestsArray = options.requestsArray;
      printer.print('requestsArray is set and will OVERRIDE any value set in requestsFilePath'.cyan);
    }
    if(options.printLoadDistribution){
      hulken.settings.printLoadDistribution = options.printLoadDistribution;
      printer.print('printLoadDistribution set to '.cyan +
        hulken.settings.printLoadDistribution.toString().bold.magenta);
    }
  }
};
