...
exports.getRandomStringOfLetters = function(numberOfChars){
  var possibleChars = 'ABCDEFGHIJKLMNOPQRSTUVXYZabcdefghijklmnopqrstuvxyz';
  return getRandomValue(possibleChars, numberOfChars);
};
exports.getRandomStringOfNumbers = function(numberOfChars){
  var possibleChars = '0123456789';
  return getRandomValue(possibleChars, numberOfChars);
};
exports.getRandomStringOfLettersAndNumbers = function(numberOfChars){
  var possibleChars = 'ABCDEFGHIJKLMNOPQRSTUVXYZabcdefghijklmnopqrstuvxyz0123456789';
  return getRandomValue(possibleChars, numberOfChars);
};
function getRandomValue(possibleChars, numberOfChars){
  var valueToReturn = '';
  for(var i = 0; i < numberOfChars; i++){
    randomIndex = Math.floor(Math.random() * possibleChars.length);
    valueToReturn += possibleChars[randomIndex];
  }
  return valueToReturn;
}
exports.getRandomValueInArray = function(list){
  randomIndex = Math.floor(Math.random() * list.length);
  return list[randomIndex];
};
exports.getCloneOfObject = function(obj){
    ...
    fs.writeFile('./options.json', JSON.stringify(
      ...
      if (err) {
        ...
        printer.print(
          ...
          .inverse);
        printer.print(
          'https://github.com/hellgrenj/hulken#settings'.white
          .inverse + '\n');
      }
    });
  }
