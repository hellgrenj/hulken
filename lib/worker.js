...
exports.runTest = function() {
  ...
  hulken.start = Date.now(); //in i en startTimer metod?
  ...
  if (hulken.settings.loginRequired) {
    hulkenAgent.post(hulken.settings.targetUrl + hulken.settings.loginUrl).send(
      ...
      try {
        ...
        printer.print('HULKEN needs a successful login to execute the requests...'.inverse.red);
        process.exit(code = 1);
      }

    });
  } else {
    printer.print_if_chatty('... no login required, continuing with anonymous user'.cyan);
    worker._move_on(index, next);
  }
}
...
worker._executeRequests = function() {
  printer.print_if_chatty('... starting to execute requests'.magenta);
...
worker._prepRequestsRecursive = function(index) {
  ...
  hulken.agents.forEach(function(hulkenAgent) {
    if (!util.stringIsInArray(hulken.settings.requestsToSkip, requestInfo.path) && !util.stringContainsChar(
...
worker._makePOSTrequest = function (request, hulkenAgent) {
  ...
  hulkenAgent.post(hulken.settings.targetUrl + request.path).send(request.payload)
    .end(function(err, res) {
      if (!err) {
        worker._handleRequestResult(res, reqStart, request, hulkenAgent, false, null);
      } else {
        worker._handleRequestResult(res, reqStart, request, hulkenAgent, true, err);
      }
    });
}
...
worker._handleRequestResult = function (res, reqStart, request, agent, failed, error) {
  ...
  if (!failed) {
    ...
    try {
      ...
      printer.print_if_chatty(('GET ' + request.path + ' resulted in ' + err).toString().red);
    }
  } else {
...
worker._printStats = function(stats) {
  ...
  printer.print('number of concurrent requests ' + stats.numberOfConcurrentRequests
    .toString().magenta);
  printer.print('number of hulken agents running ' + stats.numberOfHulkenAgents.toString()
    .magenta);
