...
worker._authenticateAgentsRecursive = function(index, next) {
  ...
  if (hulken.settings.loginRequired) {
    hulkenAgent.post(hulken.settings.targetUrl + hulken.settings.loginUrl).set(hulken.settings.headers).send(
...
worker._makePOSTrequest = function(request, hulkenAgent) {
  ...
  hulkenAgent.post(hulken.settings.targetUrl + request.path).send(request.payload).set(hulken.settings
    .headers)
