      ...
      for (var i = 0; i < hulken.settings.timesToRunEachRequest; i++) {
        hulken.requests.push(function() {
          setTimeout(function() {
            ...
            } else if (requestInfo.method.toUpperCase() ===
              'POST') {
...
worker._makePOSTrequest = function(request, hulkenAgent) {
  ...
  worker._setPostValuesSync(request);
...
worker._setPostValuesSync = function(request){
        ...
        switch (command) {
          case 'random':
            ...
            if(valueType == 'letters'){
              request.payload[property] = util.getRandomStringOfLetters(numberOfChars);
              console.log(request.payload[property]);
            }else if(valueType == 'numbers'){
              request.payload[property] = util.getRandomStringOfNumbers(numberOfChars);
              console.log(request.payload[property]);
            }else if(valueType == 'lettersandnumbers'){
              request.payload[property] = util.getRandomStringOfLettersAndNumbers(numberOfChars);
              console.log(request.payload[property]);
            }else{
...
worker._executeTestSuite = function() {
  async.each(hulken.requests, function(request, handleNextItemInAsyncEach) {
    ...
    if (asyncEachError) {
      printer.print(asyncEachError.toString().red);
    }
  });
};
